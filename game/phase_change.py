...
def start_phase_timer(game):
    ...
    game.phase_ends_at = now + timedelta(seconds=PHASE_LENGTH_SECONDS)
    game.save()
...
def advance_game_phase():
    ...
    if game.phase == 'LOBBY':
        ...
        game.winner = None
        game.save()
        add_system_message(game, "The game has started. Day begins — discuss and find the Mafia!")
    ...
    if game.phase == 'DAY':
        ...
        game.phase = 'NIGHT'
        game.save()
        add_system_message(game, "Night has begun. Mafia, choose your target.")
        start_phase_timer(game)
        return
    ...
    if game.phase == 'NIGHT':
        ...
        game.phase = 'DAY'
        game.save()
        add_system_message(game, "Day has begun. Who is suspicious?")
        start_phase_timer(game)
        return


def advance_if_timer_expired():
    """Called every second by run_game_timer. Advances phase if timer has expired."""
    game = get_game()
    if (
        game.phase in ('DAY', 'NIGHT')
        and game.phase_ends_at is not None
        and timezone.now() >= game.phase_ends_at
    ):
        advance_game_phase()